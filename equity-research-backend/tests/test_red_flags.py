import pytest
from app.graph.nodes.red_flags_calc import (check_fcf_divergence,
                                            check_high_leverage,
...
def test_check_fcf_divergence_flagged():
    f = FundamentalsSchema(
        ticker="TEST", company_name="Test", net_income=100.0, free_cash_flow=40.0
    )
...
def test_check_high_leverage_not_flagged():
    f = FundamentalsSchema(
        ticker="TEST", company_name="Test", total_debt=150.0, total_equity=100.0
    )
    res = check_high_leverage(f)
    assert res["flagged"] is False
...
def test_compute_deterministic_red_flags_all():
    f = FundamentalsSchema(
        ...
        company_name="Test",
        net_income=100.0,
        free_cash_flow=40.0,  # Flags FCF
        total_debt=300.0,
        total_equity=100.0,  # Flags Leverage
        current_assets=50.0,
        current_liabilities=80.0,  # Flags Liquidity
    )
