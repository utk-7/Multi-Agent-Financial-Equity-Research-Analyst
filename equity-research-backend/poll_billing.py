...
while datetime.datetime.now() < deadline:
    ...
    result = subprocess.run([r".\venv\Scripts\python.exe", "run_graph.py"], capture_output=True, text=True)
    print(result.stdout)
    print(result.stderr)
    
    if '"call_succeeded": true' in result.stdout or '"call_succeeded": true' in result.stderr:
        print("SUCCESS! API Key is funded and working.")
        sys.exit(0)
    elif "Your credit balance is too low" in result.stdout or "Your credit balance is too low" in result.stderr:
