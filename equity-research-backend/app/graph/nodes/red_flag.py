...
class LLMRedFlag(BaseModel):
    ...
    severity: str = Field(..., description="Severity of the flag: 'low', 'medium', or 'high'")
    description: str = Field(..., description="Detailed description citing specific numbers or narrative text")

class RedFlagLLMResult(BaseModel):
    flags: List[LLMRedFlag] = Field(..., description="List of red flags derived from reasoning. Return an empty list if none are found.")
...
"""
    ...
    logger.info(f"Red-Flag Agent completed for {ticker}. Found {len(final_flags)} flags.")
