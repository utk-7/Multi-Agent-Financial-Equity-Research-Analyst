...
def calculate_enterprise_value(
    projections: List[float], terminal_value: float, wacc: float
) -> float:
...
def perform_dcf_valuation(fundamentals: FundamentalsSchema) -> Dict[str, Any]:
    ...
    for scenario in ["base", "bull", "bear"]:
        ...
        tv = calculate_terminal_value(
            projections[-1] if projections else 0.0, wacc, config.TERMINAL_GROWTH_RATE
        )
        ev = calculate_enterprise_value(projections, tv, wacc)
    ...
    equity_values_at_base_wacc = [
        ...
        for tgr, val in sensitivity_grid.get(base_wacc_label, {}).items()
    ]
    ...
    return {
        ...
        "pre_computed_sensitivity_at_base_wacc": {
            "lowest_implied_equity": min(equity_values_at_base_wacc)
            if equity_values_at_base_wacc
            else None,
            "highest_implied_equity": max(equity_values_at_base_wacc)
            if equity_values_at_base_wacc
            else None,
        },
    }
