...
class FinalReport(BaseModel):
    executive_summary: str = Field(
        ...,
        description="High-level overview of the company's financial health, sentiment, and valuation.",
    )
    bull_case: str = Field(
        ...,
        description="The most compelling arguments for investing, based strictly on the provided data.",
    )
    bear_case: str = Field(
        ...,
        description="The most compelling arguments against investing, including valuation risks and red flags.",
    )
    synthesized_view: str = Field(
        ...,
        description="Final synthesized analysis balancing the bull and bear arguments.",
    )
    disclaimer: str = Field(
        ...,
        description="Must exactly read: 'This is research synthesis, not investment advice.'",
    )
...
async def synthesis_node(state: AgentState, config: RunnableConfig) -> Dict[str, Any]:
    ...
    dcf_json = (
        dcf_valuation.model_dump_json(indent=2)
        if hasattr(dcf_valuation, "model_dump_json")
        else json.dumps(dcf_valuation, indent=2)
    )
...
2. PRE-COMPUTED METRICS: The data now explicitly includes pre-computed derived metrics (like implied 5-year CAGR, market premium/discount, and pre-computed sensitivity extremes at the base WACC). You MAY and SHOULD cite these exact pre-computed figures directly. DO NOT manually parse the 2D sensitivity grid to derive combinations; if you discuss sensitivity, only cite the `lowest_implied_equity` and `highest_implied_equity` from `pre_computed_sensitivity_at_base_wacc`. If a pre-computed metric is `null`/not present, explicitly note it could not be calculated.
3. NO EXTERNAL KNOWLEDGE OR HALLUCINATED CATALYSTS: Do not estimate, recall figures, or introduce events, products, or macro factors from general knowledge (e.g., do not mention specific product lines unless explicitly listed in the provided news sentiment). You must construct your bull/bear cases using ONLY the provided sentiment, fundamentals, and valuations. Explicitly decline to state anything you cannot trace directly to the data.
