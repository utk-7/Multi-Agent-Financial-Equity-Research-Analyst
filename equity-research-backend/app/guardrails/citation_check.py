...
class CitationCheckResult(BaseModel):
    unsupported_claims: List[str] = Field(
        ...,
        description="List of factual claims in the report that cannot be traced to the provided data. Return empty list if all claims are supported.",
    )
    total_claims_checked: int = Field(
        ...,
        description="Total number of factual claims (numbers, ratios, quotes) found and checked.",
    )
...
async def check_citations(
    report: Dict[str, Any], state_data: Dict[str, Any]
) -> CitationCheckResult:
    """
...
"""
    ...
    try:
        result: CitationCheckResult = await _invoke_llm_with_retry(
            structured_llm, prompt
        )
        logger.info(
            f"Citation check completed. Checked {result.total_claims_checked} claims. Found {len(result.unsupported_claims)} unsupported."
        )
        return result
