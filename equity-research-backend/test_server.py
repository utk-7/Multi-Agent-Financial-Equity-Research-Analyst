...
async def test_fast_mode():
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        async with client.stream("POST", "http://127.0.0.1:8000/run", json={"ticker": "AAPL", "run_mode": "fast", "thread_id": "test_fast"}) as response:
...
async def test_verified_mode():
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        async with client.stream("POST", "http://127.0.0.1:8000/run", json={"ticker": "AAPL", "run_mode": "verified", "thread_id": "test_verified"}) as response:
            async for line in response.aiter_lines():
                if line.strip():
                    print(line)
...
async def send_approve():
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.post("http://127.0.0.1:8000/approve", json={
            "thread_id": "test_verified",
            "red_flags": [{"flagged": True, "type": "Test Flag", "severity": "low", "description": "Human approved flag", "source": "human"}]
        })
