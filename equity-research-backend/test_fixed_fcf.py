...
def test_fixed_fcf():
    for ticker in ["MSFT", "AAPL"]:
        ...
        print(f"Operating Cash Flow: {fundamentals.operating_cash_flow:,.0f}" if fundamentals.operating_cash_flow else "OCF: None")
        print(f"Free Cash Flow: {fundamentals.free_cash_flow:,.0f}" if fundamentals.free_cash_flow else "FCF: None")
