...
for ticker in ["MSFT", "AAPL"]:
    ...
    if not cf.empty:
        ...
        print("\nCash Flow Statement Latest Column (Index: ", cf.columns[0] if len(cf.columns) > 0 else "None", ")")
        ...
        if ocf_row is not None and capex_row is not None:
            print(f"Manual FCF (OCF + CapEx since CapEx is negative): {ocf_row.iloc[0] + capex_row.iloc[0]}")
